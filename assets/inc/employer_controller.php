...
if(isset($_POST['submit2'])){
  ...
  $password   = mysqli_escape_string($boom, $_POST['pwd2']);

  $sql    = "SELECT * FROM rp_employer WHERE ep_username = '$username' AND ep_password = '$password'";
  $result   = mysqli_query($boom, $sql);

 if(mysqli_num_rows($result)  > 0){
           $_SESSION['user'] = $_POST['username2'];
        header("Location: remployer/index.php");
        }else {
...
<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>x</button>
    <p style='text-trasform: capitalize;'>Please login with your correct username and password.</p>
        ...
        header("Location: signin#recent10");
        exit();
